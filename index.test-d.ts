...
const cachedPower = cache.function(async (n: number) => n ** 1000, {name: 'power'});
...
expectType<((n: string) => Promise<number>) & {fresh: (n: string) => Promise<number>}>(
	cache.function(async (n: string) => Number(n), {name: 'number'}),
);
...
expectType<Promise<number>>(cache.function(identity, {name: 'identity'})(1));
expectType<Promise<string>>(cache.function(identity, {name: 'identity'})('1'));
expectNotAssignable<Promise<string>>(cache.function(identity, {name: 'identity'})(1));
expectNotAssignable<Promise<number>>(cache.function(identity, {name: 'identity'})('1'));
...
expectType<((n: undefined[]) => Promise<undefined>) & {fresh: (n: undefined[]) => Promise<undefined>}>(
	cache.function(async (n: undefined[]) => n[1], {name: 'first'}),
);
